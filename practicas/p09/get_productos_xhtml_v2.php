...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es">
    ...
    if(isset($_GET['tope'])) {
        $tope = $_GET['tope'];
        if (!is_numeric($tope)) {
            die('No hay productos con unidades ' . htmlspecialchars($tope));
        }
    } else {
    ...
    if (!empty($tope)) {
        ...
        if ($result = $link->query("SELECT * FROM productos1 WHERE unidades <= $tope")) {
            $productos1 = $result->fetch_all(MYSQLI_ASSOC);
            $result->free();
        }
